...
public:
    ...
    /**
     * Update only the mapping table (not index_cache).
     * Use this when you need to update mapping first, then build indexes from the updated data.
     * 
     * @param artist_credit_ids Set of artist_credit_ids being updated
     * @param mapping_rows New mapping rows to insert
     * @return true on success, false on failure
     */
    bool update_mapping_only(const set<int>& artist_credit_ids,
                             const vector<MappingRowData>& mapping_rows) {
        if (artist_credit_ids.empty()) {
            lb_log("No artist_credit_ids to update");
            return true;
        ...
        lb_log("Updating mapping table for %zu artist_credit_ids (%zu mapping rows)...",
               artist_credit_ids.size(), mapping_rows.size());
        ...
        try {
            ...
            lb_error("SQLite mapping update failed: %s", e.what());
            return false;
        }
    }
    
    /**
     * Update only the index_cache table (not mapping).
     * Use this after updating mapping and building indexes from the updated data.
     * 
     * @param artist_credit_ids Set of artist_credit_ids being updated
     * @param index_blobs Map of artist_credit_id → serialized index blob
     * @return true on success, false on failure
     */
    bool update_index_cache_only(const set<int>& artist_credit_ids,
                                  const map<int, string>& index_blobs) {
        if (artist_credit_ids.empty()) {
            lb_log("No artist_credit_ids to update");
            return true;
        ...
        lb_log("Updating index_cache for %zu artist_credit_ids (%zu index blobs)...",
               artist_credit_ids.size(), index_blobs.size());
        ...
        try {
            ...
            // Delete old index_cache rows
            string delete_cache_sql = "DELETE FROM index_cache WHERE entity_id IN " + in_clause;
            int deleted_cache = db.exec(delete_cache_sql);
            lb_log("  Deleted %d old index_cache rows", deleted_cache);
            
            // Insert new index_cache rows
            if (!index_blobs.empty()) {
                SQLite::Statement insert_cache(db,
                    "INSERT INTO index_cache (entity_id, index_data) VALUES (?, ?)");
            ...
            lb_error("SQLite index_cache update failed: %s", e.what());
            return false;
        }
    }
